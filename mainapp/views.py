...
from mainapp.forms import RegisterForm, UserChangeForm,PacienteForm, CitaForm, SignoVitalForm, ConsultaForm, LaboratorioForm, RecetaForm
from django.contrib.auth import authenticate, login, logout
from django.contrib import messages
from mainapp.models import Usuario,Opciones,Paciente, CatalogoDetalle, Agendar_cita, Signos_vitales, Consulta, Laboratorio,Receta
...
def consultar_imagen(request):
    paciente_foto = Paciente.objects.raw("SELECT id_paciente, imagen FROM mainapp_paciente where id_paciente =23 ")
...
def verificar_imagen(request):
    paciente_foto = Paciente.objects.raw("SELECT id_paciente, numero_identificacion, imagen FROM mainapp_paciente where id_paciente =23 ")
    for p in paciente_foto:
    ...
    with open(filename_bio, "wb") as fh:
        fh.write(imagen64)
    mensaje= recorte_imagen(filename,filename_bio)
...
def recorte_imagen(user, user_login ):
    ...
    return ("Compatibilidad del {:.1%}".format(float(comp)))
...
def agendar_cita(request,id):
    ...
    if request.method=='POST':
        ...
        if form.is_valid(): 
            ...
            cita.save()
            return redirect('consultar_pacientes_filtro')
...
def consulta_medica(request,id):
    ...
    if request.method=='POST':
        ...
        if form.is_valid(): 
            ...
            consulta.save()
            return redirect('consultar_paciente_cita')
